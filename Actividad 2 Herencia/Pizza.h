...
class Pizza:public Comida{//1. Completa el encabezado de la clase, recuerda que deriva de Comida
    public:
        ...
        void setIngredientesExtra(std::string[], int);
        void muestra();
        void calculaPrecioFinal();

    private:
        int ingredientesExtra; // cantidad de ingredientes extra de la pizza, maximo 3
        std::string ingredientes[EXTRA_ING]; // nombre de los ingredientes
...
void Pizza::setIngredientesExtra(std::string ingredientes_[EXTRA_ING], int ingExtra_){
...
void Pizza::muestra(){
    std::cout << getNombre() << " con " << ingredientesExtra << " ingredientes extra " << std::endl;
    for (int i = 0; i < ingredientesExtra; i++)
