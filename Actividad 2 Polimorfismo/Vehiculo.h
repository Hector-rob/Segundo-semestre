...
class Vehiculo{
  ...
  // constructores
    ...
    virtual void imprimeVehiculo(); //En este método se implementará polimorfismo dinámico
...
double Vehiculo::calculaImpuestos(){
  ...
  // > 1000000 --> 25%
  double impuesto;
  if (getPrecio() >= 1 && getPrecio() <= 300000){
      impuesto = getPrecio() * IMPUESTO1;
  }

  else if(getPrecio() > 300000 && getPrecio() <= 1000000){
      impuesto = getPrecio() * IMPUESTO2;
  }

  else if (getPrecio() > 1000000){
      impuesto = getPrecio() * IMPUESTO3;
  }
